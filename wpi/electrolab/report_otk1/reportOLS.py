...
def repMoveTask(zakaz, series, spisok):
        try:
            ...
            xl.Selection.Value = u'ОЛС КТ в цеху'
            ...
            xl.Selection.Value =  u'определение\n КТ ХХ КЗ\n ОЛС в ЭТЛ'


            ws.Range('A' + str(nr) + ':E' + str(nr + len(spisok))).Select()
...
def fill_file(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer, tu,sn, idT, test, name_defect):
    ...
    #формирование словаря результатов по каждой обмотке
        ...
        hv = searchHV(item)
        if hv:
            i20 = sheet.Cells(20, 9).Value = hv[0]
...
def get_dataOLS(item):
    ...
    if query.size() == 1:
        info = {}
        model.setQuery(sql)
        info['item'] = model.index(0, 0).data()
        idsn = model.index(0, 1).data()
        info['idsn'] = model.index(0, 1).data()
        info['on'] = model.index(0, 2).data()
        info['ser'] = model.index(0, 3).data()
        info['sn'] = model.index(0, 4).data()
        info['year'] = model.index(0, 5).data()
        info['tr'] = model.index(0, 6).data()
        info['name'] = model.index(0, 7).data()
        return info
...
def fill_passportOLS_full(info):
    ...
    for i in coil['win']:
        if i == 0:
            sheet.Range(f"{sclst[position][0]}43:{sclst[position][0]}56").Value =   [[f"A-X"],[None], [None], [None], [coil['win'][i][0]], [coil['win'][i][1]], [result[0][0]],['-'],['-'],['-'],['-'],['-'],['-']]
...
def full_repOLS(item):
    wb.Worksheets(1).Activate()
    sheet = wb.ActiveSheet
    info = get_dataOLS(item)
    if not info:
        QMessageBox.warning(None, 'Не удалось найти результаты испытания', QMessageBox.Ok)
        return
    name = info['name']
    idsn = info['idsn']
    sheet.Cells(1, 3).Value = name
    sheet.Cells(2, 3).Value = f"{info['year']}-{info['sn']}"
    sheet.Cells(3, 3).Value = f"20{info['year']}"
    sheet.Cells(2, 6).Value = info['ser']
    sheet.Cells(3, 6).Value = info['on']
    ...
    sql = f"""select item.id, test_map, stand.fullname, test_map.createdatetime as date, operator.fio from item
	                    ...
	                    inner join stand  on test_map.stand = stand.id
	                    inner join operator on test_map.operator = operator.id
                        where serial_number = {info['idsn']} and stand.test_type = 17
                        order by date  desc
                        limit 1
                       """
    cor = 2
...
def fill_ticket_ols(info):
    ...
    sheet.Cells(12, 2).Value = f"{tu}      {weight}          {year}"
    sheet.Cells(5, 2).Value = f"  ном. мощность  {power}В·А"
