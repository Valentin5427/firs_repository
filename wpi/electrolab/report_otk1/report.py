...
def printing_TP(result, params, printers, controler=False):
        ...
        printer_rep = printers['report']
        printer_tick = printers['stickerTP']
        for i in ['full', 'half']:
            items = get_items(result, 'psiItem', i)
            ...
            if items:
                ...
                if 'passport' in params:
                    if reportTP.open_excel(path + 'passportTP.xlsx', printer_rep, visible=0):
                        ...
                        time.sleep(0.5)
                if 'ticket' in params:
                    if reportTP.open_excel(path + 'ticket_tp.xlsx', printer_tick, visible=1):
...
def printing_OLS(result, params, printers):
    ...
    printer_tick = printers['sticker']
    print(121212,result)
    items = get_items(result,'item_1')
    if items:
        if 'psi' in params:
            reportOLS.select_result(items)
            if reportOLS.open_excel(path+'protocolOLS.xlsx', printer_rep, visible=1):
                reportOLS.fill_psi_ols(reportOLS.fill_file)
                # printRep(self.log)

        if 'passport' in params:
            if reportOLS.open_excel(path+'passportOLS.xlsx', printer_rep, visible=1):
                reportOLS.select_resultOLS(1901732, reportOLS.fill_passportOLS_full)

        if 'ticket' in params:
            if reportOLS.open_excel(path+'ticket_ols.xlsx', printer_tick):
    ...
    if items:
        if 'psi' in params:
            reportOLS.select_result(items)
            if reportOLS.open_excel(path + 'protocolOLS.xlsx', printer_rep, visible=1):
                reportOLS.fill_psi_ols(reportOLS.fill_file_blank)
        if 'passport' in params:
            if reportOLS.open_excel(path+'passportOLS.xlsx', printer_rep, visible=1):
                reportOLS.select_resultOLS(1901732, reportOLS.fill_passportOLS_blank)
