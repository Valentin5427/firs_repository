...
# from msgbox import msgBox , getTrue
import collections
...
def float_or_int(number, digits = 1, digitsF = 2):
    ...
    number = float(number)
    if number % 1 == 0:
        print(4442, toFixed(number,digits))
        return  toFixed(number,digits)
    else:
        return toFixed(number,digitsF)


def float_or_int_2(number, digits = 1, digitsF = 2):
    if number % 1 == 0:
        return  toFixed(number,digits)

    elif get_count(number) == 1:
...
def searchPoveritel(serial):


    sql1 = f"""select operator.fio, item.createdatetime from item 
            inner join test_map on test_map.id = item.test_map
            inner join stand on stand.id = test_map.stand
            inner join operator on test_map.supervisor = operator.id
            
where item.serial_number = {serial} and stand.test_type = 1
...
def select_passportTT(items,func, kontroler = True):
    #получаем необходимые данные
    sql = f"""select
            distinct on (serial_number.serialnumber)
            t1.id,
            t1.data,
            serial_number.id,
            ...
            t1.data1
                ...
                (select item.id, serial_number  AS sn, item.acceptdatetime AS data, item.createdatetime AS data1,  poveritel.fio as fio, isptatel.fio as operator, stand.fullname as stand  from item
                ...
                order by serial_number.serialnumber
"""
    print(sql)
    model.setQuery(sql)
    if model.rowCount() < 1:
    ...
    for i in range(model.rowCount()):
        ...
        info['data1'] = model.index(i, 25).data().toString('yyyy')[2:]
...
def add_picture_keaz(sn,sheet):
    ...
    if query.first():
        ...
        if text.count('Печать для КЭАЗ'):
            ...
            print(2214, path)
            sheet.Shapes.AddPicture(path,False, True, 250, 5, 100, 30)
...
def get_params_type(type,data, fullname, idTr):
    info = {'tu':'', 'method' :'', 'nr': '', 'interval':'', 'declaration': '', 'desigation': '', 'manual': ''}
    if data == None:
        return
    model_type = QSqlQueryModel()
    data = f'{data[6:]}-{data[3:5]}-{data[0:2]}'
    sql = f"select * FROM history_type_transformer where id_type = {type} and change_date   <= '{data}' order by change_date DESC LIMIT 1 "
    model_type.setQuery(sql)
    if model_type.rowCount() == 0:
        sql = f"select * FROM history_type_transformer where id_type = {type} and change_date   > '{data}' order by change_date limit 1  "
        model_type.setQuery(sql)

    print(sql)
    info['tu'] = model_type.index(0, 2).data()
    info['method']  = model_type.index(0, 3).data()
    info['nr']  = model_type.index(0, 4).data()
    info['interval']  = model_type.index(0, 5).data()
    info['declaration'] = model_type.index(0, 8).data()
    ...
    sqlSP = f"""select transformer.type_transformer as tt, var_constr_isp as isp, climat as climat , type_transformersp.id as sp, type_transformersp.designation as designation, type_transformersp.manual as manual   from type_transformersp
                               ...
                               where type_transformersp.type_transformer = {type} and transformer.id = {idTr} order by isp DESC"""

    print(sqlSP)
    modelSP = QSqlQueryModel()
    modelSP.setQuery(sqlSP)
    climat = (None, '', modelSP.index(0, 2).data())


    for i in range(modelSP.rowCount()):
        isp = modelSP.index(i, 1).data()
        if isp in climat:
            info['desigation'] = modelSP.index(i, 4).data()
            info['manual']   = modelSP.index(i, 5).data()
            break
    return info


def fill_passportTT_full(info):
    print('заполняем паспорт ТТ')
    print(info)
    # получаем поверителя и дату изготовления
    poveritel, data2 = searchPoveritel(info['idsn'])
    typeTr = get_params_type(info['type_transformer'], data2,info['fullname'], info['idTransformer']  )
    print(434543643,typeTr)
    ...
    sheet.Cells(40, 1).Value = typeTr['tu']
    sheet.Cells(41, 4).Value = f"{info['makedate']}-{info['serialnumber']}"
    sheet.Cells(10,17 ).Value = 'Декларация о соответствии '+  typeTr['declaration']
    sheet.Cells(11, 17).Value = 'Номер в реестре СИ РФ ' + typeTr['nr']
    sheet.Cells(53, 22).Value = typeTr['interval']
    sheet.Cells(20, 21).Value = info['type']
    # sheet.Cells(44, 17).Value = f"Трансформатор тока {info['type']}  соответствует требованиям {typeTr['tu']} и признан годным для эксплуатации"
    sheet.Cells(60, 17).Value = f"Трансформатор тока {info['type']} законсервирован и упакован предприятием-изготовителем согласно требованиям Руководства по эксплуатации."
    sheet.Cells(51,17).Value = f"Трансформатор тока прошел первичную поверку по {typeTr['method']}"
    ...
    sheet.Cells(41, 12).Value = data2
    sheet.Cells(16, 20).Value = typeTr['desigation']
    sheet.Cells(40, 17).Value = f"Руководстве по эксплуатации {typeTr['manual']}"
    add_picture_keaz(info['idsn'],sheet)
    add_acceptance(info,typeTr,sheet)
    ...
    for i in coil:
        ...
        if coil[i][0] == 'Z':
            data = [ [coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]], [None], [None], [coil[i][6]], [None],  [toFixed(coil[i][9],3)], [None], [toFixed(coil[i][7],1)], [float_or_int_2(coil[i][8], 0, 4)]]
        else:
            data = [ [coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]],  [coil[i][6]], [None], [None], [None],  [toFixed(coil[i][9],3)], [None], [toFixed(coil[i][7],1)], [float_or_int_2(coil[i][8], 0, 4)] ]

        sheet.Range(f"{sclst[i][0]}44:{sclst[i][0]}58").Value = data
        position += 1
    ...
    position1 = 58
    position = 59
    dop_params = extra_options(coil, info['idItem'])
    ...
    if info['time_current'] not in (None,'',0):
        ...
        sheet.Cells(position, 7).Value = info['time_current']
        position += 1
    ...
    if check_px(info['idTransformer']):
        sheet.Range(F"A{position}:F{position + 1}").Merge()
        sheet.Range(F"G{position}:N{position + 1}").Merge()
        sheet.Cells(position, 1).Value = 'Номинальное отношение витков'
        sheet.Cells(position, 7).Value = info['turns']
        position += 2
        print(4444,coil)

    if dop_params['Rct'] != {}:
        sheet.Range(F"A{position}:F{position + 1}").Merge()
        sheet.Cells(position, 1).Value = 'Резистивное сопротивление вторичной обмотки Rct (Ом)'
        cell_merging(sheet, position, sclst, countW)
        fill_dataTC(sheet,position,sclst,'Rct',dop_params)
        position += 2
    if dop_params['Ek'] != {}:
        sheet.Range(F"A{position}:F{position + 1}").Merge()
        sheet.Cells(position, 1).Value = 'Номинальная э.д.с. в точке перегиба, Ек'
        cell_merging(sheet, position, sclst, countW)
        fill_dataTC(sheet, position, sclst, 'Ek', dop_params)
        position += 2
    if dop_params['Kr'] != {}:
        sheet.Range(F"A{position}:F{position + 1}").Merge()
        sheet.Cells(position, 1).Value = 'Коэффициент остаточной магнитной индукции Kr (%)'
        cell_merging(sheet, position, sclst, countW)
        fill_dataTC(sheet, position, sclst, 'Kr', dop_params)
        position += 2
    if dop_params['Ts'] != {}:
        sheet.Range(F"A{position}:F{position + 1}").Merge()
        sheet.Cells(position, 1).Value = 'Постоянная времени замкнутого вторичного контура Ts (сек.)'
        cell_merging(sheet, position, sclst, countW)
        fill_dataTC(sheet, position, sclst, 'Ts', dop_params)
        position += 2
    if dop_params['Ie'] != {}:
        sheet.Range(F"A{position}:F{position + 2}").Merge()
        sheet.Cells(position, 1).Value = 'Верхний предел тока намагничивания, при номинальной э.д.с. в точке перегиба, Iе'
        cell_merging(sheet, position, sclst, countW,2)
        fill_dataTC(sheet, position, sclst, 'Ie', dop_params)
        position += 3
    ...
    sheet.Cells(55, 24).Value = f"{poveritel} \n {data2}"

    #записываем контроллера
    if info['kontroler']:
        controler,data1 = searchHV(info['idItem'],10)
        sheet.Cells(47, 24).Value = f"{controler} \n {data2}"
...
def fill_passportTT_blank(info):
    ...
    sheet.Cells(65, 27).Value = info['copperP']
    sheet.Cells(66, 20).Value = info['copperPN']
    sheet.Cells(30, 20).Value = check_EAN_valid(info['serialnumber'],f"20{info['makedate']}")
    # формируем таблицу______________________________________________________
    sc = 7  # начальная координата
    sclst = {}  # словарь хранит координаты для внесения результатов обмотки
    countW = len(coil)
...
def get_data(id):
    # сначала ищм дату на поверке
    # потом на приемочном контроле
    # если нет то на последнем испытании
    #  вкрайнем случае берем актуальную дату
    for i in ([19], [23], [16,17,18,19]):
        sql = f"""select item.createdatetime from item
                inner join  test_map on item.test_map = test_map.id
                inner join stand on test_map.stand = stand.id
                where item.serial_number = {id} and stand.test_type in ({','.join(map(str,i))}) and item.istested 
                order by item.createdatetime  DESC
                LIMIT 1"""
        print(sql)
        query = QSqlQuery(sql)
        if query.first():
            data = query.value(0).toString("dd.MM.yyyy")
            return data
    return datetime.datetime.now().strftime('%d.%m.%Y')


def fill_ticket_tt(info):
    coil_name = parser_name_tt(info['idItem'])
    coil = parse_result_tt(info['idItem'],coil_name)
    print(3331, coil)
    print(43245, info)
    data = get_data(info['idsn'])
    typeTr = get_params_type(info['type_transformer'], data, info['fullname'], info['idTransformer'])
    sheet = wb.ActiveSheet
    ...
    sheet.Cells(12, 2).Value = f"{typeTr['tu']}         {info['weight']} кг 20{info['data1']} г"
...
"""
    ...
    typeTr = get_params_type(model.record(nn).field('type_transformer').value(), model.record(nn).field('createdatetime').value().toString('dd.MM.yyyy'), '',model.record(nn).field('transformer').value())
    xl.Selection.Value = typeTr['method']
...
where chk.item in (select item.id from item
        inner join test_map on test_map.id = item.test_map
        inner join stand on test_map.stand = stand.id 
where item.serial_number = {info['idSn']} and stand.test_type = 1 order by item.createdatetime DESC limit 1 )                                                                       
...
def data_TT(func):
    ...
    for i in range(model.rowCount()):
        ...
        item = model.index(i,12).data()
        shname = model.index(i,3).data()
        year = model.index(i,7).data().toString('yyyy')
        data = model.index(i,7).data().toString('dd.MM.yyyy')
        temp = int(model.index(i,8).data())
        hum = int(model.index(i,9).data())
        pres = int(model.index(i,10).data())
        idTr = model.index(i,11).data()
        oper = model.index(i,13).data()
        num_protokol = str(model.index(i, 0).data())

        sn = model.index(i, 5).data()
        test = model.index(i, 14).data()
        name_defect = model.index(i, 16).data()
        supervisor = model.index(i, 17).data()
        idsn = model.index(i, 18).data()
        type_tr = model.index(i, 19).data()
        ...
        func(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer,sn, idTr, test, name_defect, test_map,supervisor,lisnum,idsn, type_tr )
        if i < model.rowCount() - 1:
            wb.Worksheets(i + 2).Activate()
        else:
            pass

def fill_TTprotocol_full(num_protokol,item,shname,year,dataT,temp,hum,pres,oper,doc, nomer,sn, idTr, test, name_defect, test_map,supervisor,lisnum,idsn, type_tr ):
    sheet = wb.ActiveSheet
    position = 24
    coil_name = parser_name_tt(item)
    coil = parse_result_tt(item, coil_name)
    if coil == False:
        msgBox(None, 'Не удается сопоставить id обмоток трансформатора c id результатами \n распечатать пустой?')
        ...
        return
    data = get_data(idsn)
    print(343434, type_tr, data, shname,  idTr)
    typeTr = get_params_type(type_tr, data, shname,  idTr)
    sheet.Cells(9, 1).Value = f"Заводской № {nomer}"
    sheet.Cells(4, 2).Value = doc
    sheet.Cells(13, 5).Value = typeTr['tu']
    ...
    print(3242356,oper,data)
    #gпеределать
    # controler,data1 = searchHV(item,23)
    # print(12121,controler,data1
    ...
    for key in coil:
        ...
        print(3333, coil)
        # if coil[key][0] == 'D':
        #     print(999999999999999999999 ,coil[key][7]  , toFixed(coil[key][7],1))
        #     # data = [coil[key][5], None, coil[key][6],  coil[key][8], toFixed(coil[key][11],2), toFixed(coil[key][7],2)]
        #     data = [coil[key][5], None, coil[key][6], coil[key][8], toFixed(coil[key][11], 2), toFixed(coil[key][7], 2)]
        # else:
        data = [coil[key][5], None, coil[key][6],  float_or_int_2(coil[key][8], 0, 4),   toFixed(coil[key][7],2),   toFixed(coil[key][11],2) ]
    ...
    # if coil[i][0] == 'Z':
    #     data = [[coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]], [None], [None],
    #             [coil[i][6]], [None], [toFixed(coil[i][9], 3)], [None], [toFixed(coil[i][7], 1)],
    #             [float_or_int(coil[i][8], 0, 4)]]
    # else:
    #     data = [[coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]], [coil[i][6]],
    #             [None], [None], [None], [toFixed(coil[i][9], 3)], [None], [toFixed(coil[i][7], 1)],
    #             [delete_null(coil[i][8])]]


    positionN = position
    dop_params = extra_options(coil,item)
    print(2134,dop_params)
    print(3214,coil)
    # return
    if dop_params['Rct'] != {}:
        countRct = len(dop_params['Rct'])
        print(f"A{position}:A{position+countRct-1}")
        sheet.Range(f"A{position}:A{position+countRct-1}").EntireRow.Insert()
        sheet.Range(F"A{position}:B{position+countRct-1}").Merge()
        sheet.Range(f'A{position}').Value = 'Резистивное сопротивление вторичной обмотки Rct (Ом)'

        # записываем поверителя
        poveritel =  searchHV(item, 25)
        sheet.Cells(positionN, 7).Value = f"{poveritel[0]}\n {poveritel[1]}"

        if countRct == 1:
            sheet.Range(f"A{position}:A{position+countRct-1}").RowHeight = 40
        for i in  dop_params['Rct']:
            sheet.Range(F"C{position}:E{position}").Merge()
            print(886868,[coil[i][5], None,None,  [dop_params['Rct'][i]]])
            sheet.Range(f'C{position}:F{position}').Value = [coil[i][5], None,None,  dop_params['Rct'][i]]
            position +=1
    if dop_params['Kr'] != {}:
        countKr = len(dop_params['Kr'])
        print(f"A{position}:A{position + countKr - 1}")
        sheet.Range(f"A{position}:A{position + countRct - 1}").EntireRow.Insert()
        sheet.Range(F"A{position}:B{position + countRct - 1}").Merge()
        sheet.Range(f'A{position}').Value = 'Коэффициент остаточной магнитной индукции Kr (%)'
        if countKr == 1:
            sheet.Range(f"A{position}:A{position+countKr-1}").RowHeight = 40
        for i in dop_params['Kr']:
            sheet.Range(F"C{position}:E{position}").Merge()
            sheet.Range(f'C{position}:F{position}').Value = [coil[i][5], None,None,  dop_params['Kr'][i]]
            position += 1

    if dop_params['Ts'] != {}:
        countKr = len(dop_params['Ts'])
        print(f"A{position}:A{position + countKr - 1}")
        sheet.Range(f"A{position}:A{position + countRct - 1}").EntireRow.Insert(Shift=Excel_const.xlDown,
                                                                                CopyOrigin=Excel_const.xlFormatFromLeftOrAbove)
        sheet.Range(F"A{position}:B{position + countRct - 1}").Merge()
        sheet.Range(f'A{position}').Value = 'Постоянная времени замкнутого вторичного контура Ts (сек.)'
        if countKr == 1:
            sheet.Range(f"A{position}:A{position+countKr-1}").RowHeight = 40
        for i in dop_params['Ts']:
            sheet.Range(F"C{position}:E{position}").Merge()
            sheet.Range(f'C{position}:F{position}').Value = [coil[i][5], None,None,  dop_params['Ts'][i]]
            position += 1
    if dop_params['Kssc'] != {}:
        countKr = len(dop_params['Kssc'])
        print(f"A{position}:A{position + countKr - 1}")
        sheet.Range(f"A{position}:A{position + countRct - 1}").EntireRow.Insert(Shift=Excel_const.xlDown,
                                                                                CopyOrigin=Excel_const.xlFormatFromLeftOrAbove)
        sheet.Range(F"A{position}:B{position + countRct - 1}").Merge()
        sheet.Range(f'A{position}').Value = 'Cимметрический коэффициент тока короткого замыкания Kssc'
        for i in dop_params['Kssc']:
            sheet.Range(F"C{position}:E{position}").Merge()
            sheet.Range(f'C{position}').Value = i
            position += 1
    if dop_params['Ek'] != {}:
        countKr = len(dop_params['Ek'])
        print(f"A{position}:A{position + countKr - 1}")
        sheet.Range(f"A{position}:A{position + countRct - 1}").EntireRow.Insert()
        sheet.Range(F"A{position}:B{position + countRct - 1}").Merge()
        sheet.Range(f'A{position}').Value = 'Номинальная э.д.с. в точке перегиба, Ек'
        if countKr == 1:
            sheet.Range(f"A{position}:A{position + countKr - 1}").RowHeight = 40
        for i in dop_params['Ek']:
            sheet.Range(F"C{position}:E{position}").Merge()
            sheet.Range(f'C{position}:F{position}').Value = [coil[i][5], None, None, dop_params['Ek'][i]]
            position += 1
    if dop_params['Ie'] != {}:
        countKr = len(dop_params['Ie'])
        print(f"A{position}:A{position + countKr - 1}")
        sheet.Range(f"A{position}:A{position + countRct - 1}").EntireRow.Insert()
        sheet.Range(F"A{position}:B{position + countRct - 1}").Merge()
        sheet.Range(f'A{position}').Value = 'Верхний предел тока намагничивания, при номинальной э.д.с. в точке перегиба, Iе'
        if countKr == 1:
            sheet.Range(f"A{position}:A{position + countKr - 1}").RowHeight = 80
        elif countKr == 2:
            sheet.Range(f"A{position}:A{position + countKr - 1}").RowHeight = 40
        for i in dop_params['Ie']:
            sheet.Range(F"C{position}:E{position}").Merge()
            sheet.Range(f'C{position}:F{position}').Value = [coil[i][5], None, None, dop_params['Ie'][i]]
            position += 1

    if position != positionN:
        sheet.Range(F"G{positionN}:H{position-1}").Merge()
        sheet.Range(f"A{positionN}:H{position-1}").Borders.LineStyle = True
    ...
    if type_tr in (1,6):
        ...
        sheet.Range("E18:G18").Value = ['Не менее','cоответствует', f'{hv[0]} \n {hv[1]}']
...
#     select_passportTT(2248884,fill_passportTT_full)
# #
...
# # # #
# select_result(2212180)
# if open_excel('protocolTT.xlsx','hp LaserJet 1320 series (10.5.0.20)'):
#     data_TT(fill_TTprotocol_full)
...
# # 2012088
# # # # 2012089
# if open_excel('ticket_tt.xlsx', None):
#     select_passportTT(2093145, fill_ticket_tt)