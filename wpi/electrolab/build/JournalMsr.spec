...
a = Analysis([os.path.join(HOMEPATH,'support/_mountzlib.py'), 
				...
				os.path.join(ROOT_PATH, 'electrolab/gui/journalmsr.py'),
				],
             pathex=[ROOT_PATH],
             hookspath=[os.path.join(SPECPATH, 'hooks/journalmsr')]
            )
...
exe = EXE(pyz,
          a.scripts,
          exclude_binaries=1,
          name=os.path.join('build/pyi.win32/journalmsr', 'journalmsr.exe'),
          debug=False,
          strip=False,
          upx=True,
          console=False )
...
coll = COLLECT(exe,
               ...
               getTOC('./rpt', '../gui/rpt/*.xlsx', 'DATA'),
               getTOC('./rpt', '../gui/rpt/*.xml', 'DATA'),
               strip=False,
               upx=True,
               name='dist/journalmsr')