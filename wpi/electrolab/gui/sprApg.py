...
class sprApg(QDialog, UILoader):
    def __init__(self, _env, *args):
        ...
        self.setUI(_env.config, u"sprApg.ui")
        ...
        modelCB.setQuery("select  NULL, 'Все' UNION ALL SELECT id , name  from owens_apg",db1)
        self.itemsOwens  = {}
        ...
        modelCB.setQuery("select  NULL, 'Все' UNION ALL SELECT id , name  from sensors_apg", db1)
        self.itemsSensors = {}
        for i in range(modelCB.rowCount()):
            self.itemsSensors[modelCB.index(i, 1).data()] = modelCB.index(i, 0).data()
        print(self.itemsSensors)
        self.ui.comboBox_2.addItems(list(self.itemsSensors.keys()))
        ...
        self.ui.comboBox_2.activated.connect(self.change_comboBox)
...
#        print 'sortByColumn sortByColumn sortByColumn sortByColumn sortByColumn  ', columnIndex , desc
    ...
    def change_comboBox(self):
        ...
        if self.sender() == self.ui.comboBox_2:
            if self.ui.comboBox.currentIndex() != 0:

                modelCB.setQuery(
                    f"select  NULL, 'Любая' UNION ALL SELECT id , name  from sensors_apg where id_owens = {self.itemsOwens[self.ui.comboBox.currentText()]} ",
                    db1)
                self.itemsSensors = {}
                for i in range(modelCB.rowCount()):
                    self.itemsSensors[modelCB.index(i, 1).data()] = modelCB.index(i, 0).data()
                ...
                modelCB.setQuery(
                    f"select  NULL, 'Любая' UNION ALL SELECT id , name  from sensors_apg where",
                    db1)
                self.itemsSensors = {}
                for i in range(modelCB.rowCount()):
                    self.itemsSensors[modelCB.index(i, 1).data()] = modelCB.index(i, 0).data()
    ...
    def selHeart(self, columnIndex, isEdit):
        ...
        SQL = """select readings_apg.id, owens_apg.name as owens, sensors_apg.name as trm, readings_apg.set_value, readings_apg.present_value, readings_apg.date
            from readings_apg
                inner join  sensors_apg on readings_apg.id_sensor = sensors_apg.id
                inner join owens_apg on sensors_apg.id_owens = owens_apg.id 
                where readings_apg.date between to_date('""" + self.ui.dateEdit.text() + """','dd.mm.yyyy') and to_date('""" + self.ui.dateEdit_2.text() + """','dd.mm.yyyy') + 1
        """
        ...
        model.setHeaderData(1, QtCore.Qt.Horizontal, u"Печь")
        model.setHeaderData(2, QtCore.Qt.Horizontal, u"Наименование\nTRM")
        model.setHeaderData(3, QtCore.Qt.Horizontal, u"Заданная\nтемпература")
        model.setHeaderData(4, QtCore.Qt.Horizontal, u"Полученная\n температура")
        model.setHeaderData(5, QtCore.Qt.Horizontal, u"Время")
