...
def select_passportTT(items,func, kontroler = False):
...
def fill_passportTT_full(info):
    ...
    sheet.Cells(44, 17).Value = f"Трансформатор тока {info['type']}  соответствует требованиям {info['tu']} и признан годным для эксплуатации"
    ...
    for i in coil:
        ...
        print(sclst)
        if coil[i][0] == 'Z':
            data = [ [coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]], [None], [None], [coil[i][6]], [None],  [toFixed(coil[i][9],3)], [None], [toFixed(coil[i][7],1)], [ float_or_int(coil[i][8],0,4)]]
        else:
            data = [ [coil[i][5]], [None], [None], [coil[i][1]], [coil[i][2]], [coil[i][3]], [coil[i][4]],  [coil[i][6]], [None], [None], [None],  [toFixed(coil[i][9],3)], [None], [toFixed(coil[i][7],1)], [delete_null(coil[i][8])] ]
...
def fill_TTprotocol_full(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer,tu,sn, idTr, test, name_defect, test_map,supervisor,method,lisnum, type_tr):
    sheet = wb.ActiveSheet
    position = 24
    ...
    for key in coil:
        ...
        if coil[key][0] == 'D':
            ...
            data = [coil[key][5], None, coil[key][6],  coil[key][8], toFixed(coil[key][11],1), toFixed(coil[key][7],1)]
        else:
            data = [coil[key][5], None, coil[key][6],  coil[key][11] , toFixed(coil[key][7],1), toFixed(coil[key][8],4)]
