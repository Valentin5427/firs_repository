...
class INI(common.INI):
    ...
    def error(self):
        ...
        if (not self.opened):
            raise Exception(u'Not open file %s', self.config.filename)
        return self.get_optional_value(self.section, u'Erorr')
...
class ExchangeFile():
    ...
    def __init__(self, _sPath, _sFileName, _sArchPath=None, _sErorrPath = None):
...
class FileSystem():
    ...
    def get_filelist(self, _sPath, _sArchPath=None, _sErrorPath = None):
        ...
        for root, dirs, files in os.walk(_sPath):
            ...
            for file in files:
                if ExchangeFile(root, file).correct:
                    lst.append(ExchangeFile(root, file, _sArchPath, _sErrorPath))
        return lst
