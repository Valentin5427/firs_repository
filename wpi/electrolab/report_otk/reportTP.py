...
def select_passportTP(items,func, kontroler = True):
...
"""
    ...
    for i in range(model.rowCount()):
        ...
        info['ktt'] = model.index(i, 24).data()
...
def add_acceptance(info, typeTr,sheet):
    ...
    if query.first():
        ...
        if text.count('Морской Регистр'):
            sheet.Cells(45,
                        17).Value = f"""Трансформатор тока {info['type']}  соответствует требованиям {typeTr['tu']}, требованиям п.2.1.1 раздела 2 Правил классификациии постройки и морских судов (часть XI) и признан годным для эксплуатации"""
        else:
            sheet.Cells(45,
...
def fill_passportTP_full(info):
    ...
    typeTr = get_params_type(info['type_transformer'], info['data'], info['fullname'], info['idTransformer'])
    ...
    # sheet.Cells(45,17).Value = f"Трансформатор тока {info['type']}  соответствует требованиям {typeTr['tu']} и признан годным для эксплуатации"
...
def fill_ticket_tp(info):
    ...
    sheet.Cells(6, 2).Value = typeTr['tu']
    sheet.Cells(6, 13).Value = f"20{info['data1']} г"
    return
...
def fill_TPprotocol_full(num_protokol, item, shname, year, data, temp, hum, pres, oper, doc, nomer, sn, idTr, test, name_defect,test_map, supervisor, lisnum, idsn, type_tr):
    sheet = wb.ActiveSheet
    position = 22
