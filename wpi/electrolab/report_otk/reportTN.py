...
def parser_name_tt(item):
        ...
        for i in winding.keys():
            if winding[i]['coilnumber'] == 0 and  winding[i]['tap'] == 0:
                ...
                idPerv = i
                continue
            elif winding[i]['class'] in ('3P','6P'):
                if sumD == 1:
                    text = ['aд-xд', winding[i]['class'],  winding[i]['secload'],   sqrtPOV[winding[i]['seccur']],winding[i]['coilnumber']]
                else:
                    text = [f"aд{sD}-xд{sD}",  winding[i]['class'] , winding[i]['secload'], sqrtPOV[winding[i]['seccur']],winding[i]['coilnumber']]
                    sD += 1

            else:
                if sumI == 1:
                    text = ['a-x', winding[i]['class'] , winding[i]['secload'], sqrtPOV[winding[i]['seccur']],winding[i]['coilnumber']]
                else:
                    text = [f"a{sI}-x{sI}", winding[i]['class'] , winding[i]['secload'], sqrtPOV[winding[i]['seccur']],winding[i]['coilnumber']]
                    sI += 1
...
def select_passportTN(items,func,controler = True):
    ...
    for i in range(model.rowCount()):
        ...
        info['data1'] = model.index(i, 24).data().toString('yyyy')[2:]
...
def add_acceptance(info, typeTr,sheet):
    ...
    if query.first():
        ...
        if text.count('Морской Регистр'):
            sheet.Cells(43,
                        17).Value = f"""требованиям {typeTr['tu']}, требованиям п.2.1.1 раздела 2 Правил классификациии постройки и морских судов (часть XI) и признан годным для эксплуатации"""
        else:
            sheet.Cells(43,
                        17).Value = f"требованиям {typeTr['tu']} и признан годным для эксплуатации"



def get_params_type(type,data, fullname, idTr):
    info = {'tu':None, 'method' :None, 'nr': None, 'interval':None, 'declaration': None, 'desigation': None, 'manual': None}
    if data == None:
        return
...
def fill_passportTN_full(info):
    ...
    sheet.Cells(39, 1).Value = typeTr['tu']
    # sheet.Cells(43, 20).Value = typeTr['tu']
    ...
    sheet.Cells(52, 22).Value = typeTr['interval']
    sheet.Cells(21, 21).Value = info['type']
    sheet.Cells(42, 25).Value = info['type']
    sheet.Cells(59, 25).Value = info['type']
    sheet.Cells(55, 24).Value = info['data']

    sheet.Cells(64, 27).Value = info['copperP']
    sheet.Cells(65, 20).Value = info['copperPN']
    sheet.Cells(51, 17).Value = typeTr['method']
    sheet.Cells(16, 20).Value = typeTr['desigation']
    sheet.Cells(37, 23).Value = typeTr['manual']

    # записываем поверителя
    sheet.Cells(54, 24).Value = f"{poveritel} \n {data2}"
    sheet.Cells(40, 12).Value = data2
    if info['controler']:
        # записываем контроллера
        controler, data1 = searchHV(info['idItem'], 10)
        sheet.Cells(46, 24).Value = f"{controler} \n {data2}"
    ...
    if info['type_transformer'] in (10, 11):
        ...
        try:
            ...
            print(5767676767, ex)
            QMessageBox.warning(None, f"Предупреждение1",
        ...
        try:
            ...
            QMessageBox.warning(None, f"Предупреждение2",
                                f"Отсутствуют данные холостого хода трансформатора  {info['fullname']} {ex}", QMessageBox.Ok)
    ...
    for tr in range(model.rowCount()):
        ...
        if result:
            ...
            if test in ('R', 'RP'):
                ...
                positionY += 1
            print(75678567597947, test, test in ('P','RP') )
            if test in ('P','RP'):
                print(75678567547)
                if type_load:
...
def data_TN(func):
    ...
    for i in range(model.rowCount()):
        ...
        func(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer,sn, idTr, test, name_defect, test_map,supervisor, lisnum,type_tr )
        if i < model.rowCount() - 1:
            wb.Worksheets(i + 2).Activate()
        else:
            pass

def fill_TNpoverka_full(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer, sn, idTr, test, name_defect, test_map,supervisor,lisnum, type_tr  ):
    sheet = wb.ActiveSheet
    try:
        ...
        E18 = sheet.Cells(18, 5).Value = typeTr['method']
...
def fill_TNprotocol_full(num_protokol,item,shname,year,data,temp,hum,pres,oper,doc, nomer, sn, idTr, test, name_defect, test_map,supervisor,lisnum, type_tr ):
    result = resultDictTT(item, shname,nomer)
    ...
    if result:
        ...
        # записыв
        sql = f"""select * from 
    (select  isolation_level ,prime_test_voltage, second_test_voltage, prime_test_voltage2,
        CASE 
            WHEN transformer.isolationlevel IS  NULL and isolation_level = 'б'  THEN TRUE
            WHEN transformer.isolationlevel = 'a' and isolation_level = 'a'  THEN TRUE
            WHEN transformer.isolationlevel = 'б' and isolation_level = 'б'  THEN TRUE
            end type_level
        from testing_voltage inner join transformer on testing_voltage.nominal_voltage = transformer.voltage
        where transformer.id = {idTr})t1
    where type_level 
                   """
        print(sql)
        ...
        print(corT)
        sheet.Range(corT).EntireRow.Insert()
        print(31314235235, position,position+count)
        sheet.Range(f"A{position}:B{position+count}").Merge()
...
def get_data(id):
    ...
    for i in ([19], [23], [16,17,18,19]):
        ...
        if query.first():
            ...
            print(32321, data)
            return data
    print(24256,datetime.datetime.now().strftime('%d.%m.%Y') )
...
def fill_ticket_tn(info):
    ...
    sheet.Cells(12, 2).Value = f"{typeTr['tu']}     {int(info['weight'])} кг  20{info['data1']}г"
...
#     select_passportTN(2257453,fill_passportTN_full)

# # if open_excel('poverkaTN.xlsx','hp LaserJet 1320 series (10.5.0.20)' , visible= 0):
# #     data_TN(fill_TNpoverka)
# #     printRep()
# # #
# select_result(2225893)
# if open_excel('protocolTN.xlsx','hp LaserJet 1320 series (10.5.0.20)' , visible= 1):
#     data_TN(fill_TNprotocol_full)
# open_excel('stickerTTP.xlsx', None)
# select_Stikert(2239638)
# stickerTTP('RP')